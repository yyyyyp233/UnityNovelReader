...
Settings 中的 `Strong hover disguise` 默认关闭。开启后，鼠标在窗口内时显示所选 Reader 或 Settings；鼠标移出窗口或 Unity 失去焦点时，标题、工具栏和内容会整体切换成 Console，并强制显示 Synthetic Header 与配套日志详情。目录、章节搜索词、`Clear` 状态和普通滚动位置不会泄露到这个伪装视图。强力伪装开启期间伪装键不再额外切换画面，老板键保持原行为。
...
源码包支持 Unity 2020.3 或更高版本。EditMode 测试位于 `Tests/Editor`。`v1.0.0` 已在 Unity `2022.3.62f2` 完成导入、编译和 59 项 EditMode 测试，其中 58 项通过、1 项因环境条件忽略；真实 GB18030 长篇文本 smoke test 识别到 1,840 章。
