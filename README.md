...
> 项目定位：给 Unity 开发者在工作间隙摸鱼看本地小说用的轻量小工具。Console 伪装和快捷隐藏是主要功能，不以通用电子书阅读器为目标。
...
- 老板键与伪装键已分开：老板键负责隐藏阅读器并切换到目标窗口；伪装键在 Console 模式下只切换小说正文与配套日志详情，在 Classic 模式下等同老板键。
- 老板键会优先切换同一 Dock 内的其他页签，恢复阅读器时保持原来的停靠布局；无法切换同组页签时才关闭窗口。
- 新增默认关闭的 `Strong hover disguise`：鼠标移出阅读器或 Unity 失去焦点后，Console、Classic 和 Settings 都会临时显示完整 Console 伪装；鼠标返回后恢复原界面与阅读位置。
- Console Log 行支持右键添加一次性蓝色定位标记，翻页、跳章、换书或标记其他行后自动清除，不写入书签。
- Reimport 后会保留当前页与滚动位置；进入或退出 Play Mode 后，Console 级别图标会自动恢复。
- Synthetic Header 的时间和候选消息只在翻页、跳章、换书或重开窗口等明显变化时更新；Info、Warning、Error 候选池分别为 25、15、5 条。
- `Open TXT`、书库、阅读皮肤切换和 Settings 统一收纳在 `Editor ▼` 菜单，Classic 与 Console 模式可以双向切换。
...
- 目录/书签侧栏默认收起，需要时可以展开。
- 带时间戳的伪日志头可选，默认关闭。
- 伪装键可将小说正文切换为与每条伪日志头一一配套的日志详情，再次触发会恢复正文。
- 可选的强力伪装只在鼠标位于阅读器窗口内时显示小说；鼠标移出或 Unity 失去焦点后，整个窗口会自动切换为完整 Console 伪装。
...
- 提供可重绑的伪装键、老板键和可选伪装窗口。
- 主窗口内置轻量 Settings 页面，可设置快捷键、阅读皮肤、伪日志头和伪装策略。
- 强力伪装在 Console、Classic 和 Settings 页面均生效，并使用独立滚动位置，不影响小说阅读位置。
...
1. 从 [最新 Release](https://github.com/yyyyyp233/UnityNovelReader/releases/latest) 下载 `UnityNovelReader-1.0.0.unitypackage`。
...
powershell -ExecutionPolicy Bypass -File .\Tools~\Build-UnityPackage.ps1 `
  ...
  -OutputPath ".\Releases~\UnityNovelReader-1.0.0.unitypackage"
```
...
Settings 中的 `Strong hover disguise` 默认关闭。开启后，鼠标在窗口内或重新选中阅读器时显示所选 Reader 或 Settings；鼠标移出窗口或 Unity 失去焦点时，标题、工具栏和内容会整体切换成 Console，并强制显示 Synthetic Header 与配套日志详情。目录、章节搜索词、`Clear` 状态和普通滚动位置不会泄露到这个伪装视图。强力伪装开启期间伪装键不再额外切换画面，老板键保持原行为。无论当前是否正在伪装，都可以通过阅读器的 `Editor ▼ > Strong Hover Disguise` 直接关闭；若窗口状态异常，也可以使用 Unity 顶部菜单 `Tools > Unity Novel Reader > Disable Strong Hover Disguise` 恢复。
...
| 伪装键：Console 模式切换正文/日志详情；Classic 模式等同老板键 | `Ctrl+Alt+R` |
...
通过 `Editor ▼ > Boss-key target` 选择老板键目标：`Scene`、`Console`、`Profiler`、`Animator` 或 `Project`。老板键在阅读器与其他页签位于同一个 Dock 区域时会切换同组页签并保留原窗口实例，恢复后仍处于原来的页签布局。强力伪装关闭时，伪装键在 Console 模式下只切换正文显示、不移动窗口，在 Classic 模式下行为等同老板键。Console 模式可通过 `Editor ▼ > Classic Reader` 切换到传统布局；Classic 模式保留同一个 `Editor ▼` 菜单，并显示 `Console Reader` 用于切回伪装模式。如果当前 Unity 版本无法打开所选窗口，会自动回退到 `Scene`。
...
源码包支持 Unity 2020.3 或更高版本。EditMode 测试位于 `Tests/Editor`。当前版本已在 Unity `2022.3.62f2` 完成导入、编译和 64 项 EditMode 测试，其中 63 项通过、1 项因环境条件忽略；真实 GB18030 长篇文本 smoke test 识别到 1,840 章。
