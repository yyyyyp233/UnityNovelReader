...
以下调整已进入当前 `main`。`v0.2.4` 标签仍指向调整前的发行版；需要这些功能时可使用 `main` 源码或在本地重新构建 `.unitypackage`。
...
源码包支持 Unity 2020.3 或更高版本。EditMode 测试位于 `Tests/Editor`。当前 `main` 已在 Unity `2022.3.62f2` 完成导入、编译和 59 项 EditMode 测试，其中 58 项通过、1 项因环境条件忽略；真实 GB18030 长篇文本 smoke test 识别到 1,840 章。
