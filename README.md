...
- `Open TXT`、`Prev`、`Next`、`Mark`、`Editor ▼` 分别提供文件、翻页、书签、面板和设置功能。
...
通过 `Editor ▼ > Boss-key target` 选择老板键目标：`Scene`、`Console`、`Profiler`、`Animator` 或 `Project`。`Editor ▼ > Classic Reader` 可以恢复原阅读布局，并可从其 Settings 页面切回 Console 皮肤。如果当前 Unity 版本无法打开所选窗口，会自动回退到 `Scene`。
